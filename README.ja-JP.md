...
- 🎐 **静寂の瞬間** — スペースキーで世界が止まる：数字が凍り、心音が止み、「今、世界はあなたの止めた一秒の中で、続いています」の一言が闇の中に浮かぶ。滞在が1分 / 5分 / 15分を超えるたび、画面があなたのリアルなセッション計数を一度だけ囁く
- 🖋 **巡る詩** — サブタイトルは固定の一文ではなく、手書きの三言語アフォリズムを88秒ごとにクロスフェードで輪番。オープニングのカメラは UI 言語に応じた地域へ飛び、地球は最初の一眼であなたを「覚えている」
...
│   ├── i18n.js              # 三言語コピー
│   ├── voice.js             # ページの「声」: 巡る詩 / 静寂の瞬間 / マイルストーンの囁き
│   ├── humanize.js          # セッション純増数の人間スケール変換
│   ├── news.js              # スクロール速報の生成
